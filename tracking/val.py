...
class Evaluator:
                ...
                with zipfile.ZipFile((val_tools_path / (benchmark + '.zip')), 'r') as zip_file:
                    if opt.benchmark == 'MOT16':
                        ...
                        for member in tqdm(zip_file.namelist(), desc=f'Extracting {benchmark}'):
                            if os.path.exists(val_tools_path / 'data' / 'MOT16' / member) or os.path.isfile(
                                    val_tools_path / 'data' / 'MOT16' / member):
                                pass
                            else:
                                zip_file.extract(member, val_tools_path / 'data' / 'MOT16')
                    else:
                        for member in tqdm(zip_file.namelist(), desc=f'Extracting {benchmark}'):
                            if os.path.exists(val_tools_path / 'data' / member) or os.path.isfile(
                                    val_tools_path / 'data' / member):
                                pass
                            else:
    ...
    def eval(self, opt, seq_paths, save_dir, MOT_results_folder, val_tools_path, gt_folder, free_devices):
        ...
        results = (
            save_dir.parent / self.opt.eval_existing / 'tracks' if self.opt.eval_existing else save_dir / 'tracks').glob(
            '*.txt')
        for src in results:
